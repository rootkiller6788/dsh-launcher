...
**2.1 待补**：§5 风险里写的"崩溃规则可外部覆盖"**尚未做**（原文说"沿用 AHL 已有的
`DSH_BOOT_SIGNATURES` 式做法"——该机制在本仓库中并不存在，属当时的目标设想而非现状）。
现在规则是编译期常量，dsh 改措辞就得重新发版。
...
| **`3/zat` 的崩溃规则会腐坏** | 那 13 类规则绑定 dsh 0.6.x~1.5.7 的具体报错文本 | 规则表要可外部覆盖，不硬编码进二进制。**2026-09-12 核实**：原文所说的 "AHL 已有的 `DSH_BOOT_SIGNATURES` 式做法" 在本仓库中**并不存在**，属设想而非现状；规则表目前是 `crash.rs` 里的编译期常量，这项缓解措施仍待实现（见 §2 Phase 2 实施记录"2.1 待补"） |
